...
## 프로젝트 개요
터미널 명령어 딱 한 줄로 수십 대의 가상 아이폰을 생성하고 조작할 수 있는 "맥OS 전용 가상 iOS 커맨드라인 관제탑"
스타 1.3만 개를 보유한 인기 도구로, 무거운 시뮬레이터 창을 일일이 띄우지 않고도 터미널 스크립트로 모바일 앱 테스트와 자동화를 일괄 처리
모바일 앱 개발자와 QA 엔지니어의 작업 시간을 수십 배 단축시켜 주는 강력하고 날렵한 가상 기기 제어 도구
